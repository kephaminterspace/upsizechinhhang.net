...
jQuery(function ($) {
    ...
    $(window).scroll(function () {
        ...
        if (top >= ($("#effetcs").offset().top) - 40) {
            $("header").addClass("active");
        } else {
            $("header").removeAttr("class");
        }
//        console.log($("#effetcs").offset().top);
    });
    $("#top-menu ul li").click(function () {
        var array_sections;
        var index = $(this).index();
        array_sections = ["#info", "#thanhphan", "#effetcs", "#spec-tell", "#form"];
        if (index == ($("#top-menu li").length))
            index = index - 1;
        $('html,body').animate({scrollTop: $(array_sections[index]).offset().top - $("header").height()}, 400);
        return false;
    });

    $("#newMenu ul li").click(function () {
        var array_sections;
        var index = $(this).index();
        array_sections = ["#info", "#thanhphan", "#effetcs", "#spec-tell", "#form"];
        if (index == ($("#top-menu li").length))
    ...
    $(".select").click(function () {
        if ($(this).hasClass("active"))
            $(this).removeClass("active").find(".select-drop").hide();
        else
            $(this).addClass("active").find(".select-drop").show();
        return false;
    });
